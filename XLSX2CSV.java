...
==================================================================== */
// Bron: http://svn.apache.org/repos/asf/poi/trunk/src/examples/src/org/apache/poi/xssf/eventusermodel/XLSX2CSV.java
...
import static org.apache.poi.util.SAXHelper.newXMLReader;
...
public class XLSX2CSV {
    ...
    private class SheetToCSV implements SheetContentsHandler {
        ...
        final public char scheidingsteken = '|';

        private void outputMissingRows(int number) {
            for (int i = 0; i < number; i++) {
                for (int j = 0; j < minColumns; j++) {
                    output.append(scheidingsteken);
                }
                output.append('\n');
            }
        }
        ...
        public void cell(String cellReference, String formattedValue,
            ...
            try {
                ...
                parseDouble(formattedValue);
                output.append(formattedValue);
            } catch (NumberFormatException e) {
                // Vervang dubbel aanhalingsteken (34) door enkel aanhalingsteken (39(
                if (formattedValue.contains(Character.toString((char) 34)) ) {
                    formattedValue = formattedValue.replace((char) 34, (char) 39);
                }
                // Vervang puntkomma ';' door komma ',' 
                if (formattedValue.contains(Character.toString(';')) ) {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 1) {
            err.println("Use:");
            err.println("  XLSX2CSV <xlsx file> [min columns]");
            return;
        }

        File xlsxFile = new File(args[0]);
        if (!xlsxFile.exists()) {
            err.println("Not found or not a file: " + xlsxFile.getPath());
            return;
        }

        int minColumns = -1;
        if (args.length >= 2) {
            minColumns = parseInt(args[1]);
        }

        try ( // The package open is instantaneous, as it should be.
                OPCPackage p = open(xlsxFile.getPath(), READ)) {
